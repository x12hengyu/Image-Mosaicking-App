...
%BLENDIMAGEPAIR
    ...
    for c = 1:Cs
        ...
        elseif strcmp(mode, 'blend')
            ...
            weighted_masks = bwdist(~binary_mask_s);
            weighted_maskd = bwdist(~binary_mask_d);
            
            weights = weighted_masks ./ max(weighted_masks(:));
            weightd = weighted_maskd ./ max(weighted_maskd(:));
            
            % I_blend = (w1*i1 + w2*i2) / w1+w2
            channel_out = (weights .* S + weightd .* D) ./ (weights + weightd);
