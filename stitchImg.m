...
%               get you started.
    ...
    for n = 1:num_imgs
        ...
        [~, H_3x3] = runRANSAC(kp_stitched, kp_n, 300, 1);

        % back wrap
        src_width_height = [size(stitched_img,2), size(stitched_img,1)];
        [src_mask, src_img] = backwardWarpImg(img_n, H_3x3, src_width_height);
    
        % blend
        stitched_img(isnan(stitched_img)) = 0;
        stitch_mask = rgb2gray(stitched_img) > 0;
        stitched_img = blendImagePair(src_img, src_mask, stitched_img, stitch_mask, 'blend');
